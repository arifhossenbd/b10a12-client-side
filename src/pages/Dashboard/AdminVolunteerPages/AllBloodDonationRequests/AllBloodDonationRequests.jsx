...
const AllBloodDonationRequests = () => {
  ...
  const {
    ...
    `/blood-requests?role=${role}&page=${currentPage}&limit=${itemsPerPage}`
  );

  const allRequest = responseData?.data || [];
  ...
  const handleDeleteRequest = async (requestId) => {
    try {
      ...
      const response = await axiosPublic.delete(
        `/blood-requests/${requestId}?email=${user?.email}&role=${role}`
      );
              ...
              <tbody>
                <AnimatePresence>
                  {allRequest?.map((request) => {
                    ...
                    const isPending = request?.status?.current === "pending";
                    const isCancelled =
                      ...
                      >
                        ...
                        <td className="pr-4 md:pr-6">
                          <div className="flex justify-end gap-1 md:gap-2">
                            ...
                            {/* Admin or requester actions */}
                            {(isAdmin || isRequester || isVolunteer) && (
                              <>
                                {isPending && (
                                  <>
                                    {/* Edit btn */}
                                ...
                                {isInProgress && (
                                  <>
                                    ...
                                    <motion.button
                                      ...
                                      className="btn btn-ghost btn-xs text-success hover:text-success tooltip tooltip-success tooltip-left"
                                      data-tip="Complete"
                                    >
