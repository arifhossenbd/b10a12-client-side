...
import { FaChartLine, FaUsers, FaHandsHelping, FaHistory, FaTint } from "react-icons/fa";
...
const Dashboard = () => {
  ...
  const currentStats = statsConfig[role?.toLowerCase()] || statsConfig.volunteer;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        ...
        />
        <HeaderStats stats={currentStats.map(stat => ({
          label: stat.title,
          value: stat.value,
          color: stat.color
        }))} />
      ...
      >
        ...
        >
          Welcome back, <span style={{ color: COLORS.primary }}>{userData?.name}</span>
        </h1>
        <p 
          className="mt-2"
          style={{ color: COLORS.textSecondary }}
        >
          {role === 'volunteer' && "Here's an overview of your volunteer activities."}
          {role === 'admin' && "Here's your system overview and quick actions."}
          {role === 'donor' && "Thank you for being a life saver! Here's your donation summary."}
        </p>
      </motion.div>
